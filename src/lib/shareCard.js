export async function renderShareCard({ name, time, avatar }) {
    ...
    ctx.fillStyle = "#ffffff"; ctx.textAlign = "left"; ctx.font = "800 360px 'Seven Segment', system-ui, sans-serif";
    ...
    ctx.fillStyle = "white"; ctx.textAlign = "center"; ctx.font = "800 152px Inter, system-ui, sans-serif";
...
function drawCircularImage(ctx, src, cx, cy, radius, border = 0, borderColor = "white") {
    ...
    return new Promise((resolve) => {
        img.onload = () => {
            ...
            ctx.restore();
            if (border > 0) {
                ctx.beginPath(); ctx.arc(cx, cy, radius + border / 2, 0, Math.PI * 2);
